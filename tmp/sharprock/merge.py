...
def remove_pts(input_string):
    # Remove "(15pts)", "(M...)", and "(WA...)" patterns
    pattern = r'\([^)]+pts\)|\(\w+\)|\(\w+[^)]+\)'
    
    # Remove subsequent white spaces
    result_string = re.sub(pattern, '', input_string)
    result_string = re.sub(r'\s+', ' ', result_string)  # Remove subsequent white spaces
    
    return result_string.strip()  # Remove leading and trailing white spaces
...
def get_id_prefix_title_maps(filename):
    ...
    for zone in data['zones']:
        ...
        try:
            ...
            for qq in zone['questions']:
                for question in qq['alternatives']:
                    qid = question['id']
                    prefix = get_prefix(qid)
                    title = zone['title']
                    id_prefix_map[qid] = prefix
...
def generate_homework_json(ids):
    ...
    homework = {
        "uuid": "7f36ab08-462f-4168-953c-1c6fc1441e45",
        "type": "Homework",
        "title": "Midterm 2",
        "set": "Sharp Rocks",
        "number": "14",
...
def extract_zone_titles(json_obj):
    ...
    for zone in zones:
        ...
        if title is not None:
            print(' ==== ')
            print(title)
            print(remove_pts(title))
            titles.append(remove_pts(title))
    return titles
